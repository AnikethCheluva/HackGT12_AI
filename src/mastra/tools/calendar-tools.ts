...
const addCalendarEventTool = createTool({
    ...
    execute: async ({ context }) => {
        ...
        return { success: true, eventId: createdEvent.data.id || undefined };
    }
});
...
const updateCalendarEventTool = createTool({
    ...
    execute: async ({ context }) => {
        ...
        const updatedEvent = {
            ...event.data,
            start: context.newDate && context.newTime ? { dateTime: new Date(`${context.newDate}T${context.newTime}`).toISOString() } : event.data.start,
            end: context.newDate && context.newTime ? { dateTime: new Date(new Date(`${context.newDate}T${context.newTime}`).getTime() + 3600000).toISOString() } : event.data.end, // default to 1 hour later
